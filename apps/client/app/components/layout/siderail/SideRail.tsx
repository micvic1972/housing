...
import base from "./SideRail.module.css";
import wide from "./SideRail.wide.module.css";
...
export default function SideRail() {
  ...
  return (
    <nav className={cx(base.rail, wide.rail)} aria-label="Primary">
      {NAV_ITEMS.map((item) => {
        ...
        return (
          <Link
            key={item.id}
            href={item.href}
            className={cx(base.item, wide.item)}
            aria-current={active ? "page" : undefined}
          >
            <Icon size={22} strokeWidth={active ? 2.3 : 1.8} className={base.icon} />
            <span className={cx(base.label, wide.label)}>{item.label}</span>
          </Link>
        );
      })}
    </nav>
